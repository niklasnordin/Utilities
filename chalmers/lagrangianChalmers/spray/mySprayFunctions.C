...
) const
{
    ...
    {
        scalar dist = (elmnt().position() - pos) & normal/mag(normal);
        if (dist < thickness)
        {
            label celli = elmnt().cell();
            scalar Pc = p()[celli];
...
scalarList spray::nd3_nd2_AtPlane
(
    const vector& pos,
    const vector& normal,
    const scalar thickness
) const
{
    ...
    scalar dist = (elmnt().position() - pos) & normal/mag(normal);
        if (dist < thickness)
        {
            ...
            scalar tmp = elmnt().N(rho)*pow(elmnt().d(), 2.0);
            nd3_nd2[0] += tmp*elmnt().d();
            nd3_nd2[1] += tmp;
        }
    }
    reduce(nd3_nd2[0], sumOp<scalar>());
    reduce(nd3_nd2[1], sumOp<scalar>());

    return nd3_nd2;
}

scalarList spray::nd3_nd2_AtRing
(
    const vector& pos,
    const vector& normal,
    const scalar radiusMax,
    const scalar thickness
) const
{
    scalarList nd3_nd2;
    ...
    {
    scalar dist = mag(elmnt().position() - pos);
    scalar distNormal = (elmnt().position() - pos) & normal/mag(normal);
    scalar distHoriz = sqrt(sqr(dist)-sqr(distNormal));
        if (distNormal < thickness)
        {
            if ( (distHoriz < radiusMax) && (distHoriz > (radiusMax - thickness)) )
            {
                label celli = elmnt().cell();
                scalar Pc = p()[celli];
                scalar T = elmnt().T();
...
scalarList spray::nd3_nd2_AtRings
(
    const vector& pos,
    const vector& normal,
    const scalarList radiusMaxis,
    const scalar thickness
) const
{
    scalarList nd3_nd2;
    nd3_nd2.setSize(2*radiusMaxis.size());

    for(label i = 0; i < nd3_nd2.size(); ++i)
   	 { if(i % 2 == 0)
    		nd3_nd2[i] = 0.0;
    	   else nd3_nd2[i] = VSMALL;
         }
    for
    (
        spray::const_iterator elmnt = begin();
    ...
    scalar distHoriz = sqrt(sqr(dist)-sqr(distNormal));
        if (distNormal < thickness)
        {
        for(label i = 0; i < radiusMaxis.size(); ++i){  
            if ( (distHoriz < radiusMaxis[i]) && (distHoriz > (radiusMaxis[i] - thickness)) )
            {
                label celli = elmnt().cell();
                scalar Pc = p()[celli];
                scalar T = elmnt().T();
    ...
    for (label i = 0; i<nd3_nd2.size(); ++i)
    {    reduce(nd3_nd2[i], sumOp<scalar>());
    }

    return nd3_nd2;
}

scalarList spray::nd3_nd2_AtPoints
(
    const vectorList& pos,
    const scalar radius
) const
{
    scalarList nd3_nd2;
    nd3_nd2.setSize(pos.size()*2);
    ...
    scalarList dist;
    dist.setSize(pos.size());
    for(label i = 0; i < dist.size(); ++i)
    {
        dist[i] = 0.0;
    }
//    scalar numerator = 0.0, denominator = VSMALL;
    ...
    for(label i = 0; i < dist.size(); ++i)
    {    
        dist[i] = mag(elmnt().position() - pos[i]);
        if (dist[i] < radius)
        {
            label celli = elmnt().cell();
            scalar Pc = p()[celli];
...
scalarList spray::dropletCountsAtRing
(
    const vector& pos,
    const vector& normal,
    const scalar radiusMax,
...
) const
{
    ...
    scalar deltaD = 2e-6;
    label numCounts = (dMax-dMin)/deltaD;

    scalarList dropletCountsAtRing;
    dropletCountsAtRing.setSize(numCounts); 

    for(label i = 0; i < numCounts; ++i)
    { 
        dropletCountsAtRing[i] = 0;
    }
    ...
    scalar distHoriz = sqrt(sqr(dist)-sqr(distNormal));
        ...
        if (distNormal < thickness)
        {
            if ( (distHoriz < radiusMax) && (distHoriz > (radiusMax - thickness)) )
            {
                for(label i = 0; i < numCounts; ++i)
                {
                    if((elmnt().d() > (dMin + deltaD*i)) && (elmnt().d() < (dMin + deltaD*(i+1))))
                    {
                        label celli = elmnt().cell();
                        scalar Pc = p()[celli];
                        scalar T = elmnt().T();
                        ...
                        dropletCountsAtRing[i] += elmnt().N(rho); 
                    }
    ...
    for (label i = 0; i < numCounts; ++i)
    {    reduce(dropletCountsAtRing[i], sumOp<scalar>());
    }

    return dropletCountsAtRing;
}
